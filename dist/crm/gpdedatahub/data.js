...
const formatAction = (queueAction, config) => {
    ...
    /// Add data form_type
    data.form_type =
        queueAction.action.actionType === "mail2target" ||
            queueAction.action.actionType === "email"
            ? "protestmail"
            : "petition";
    if (data.action.customFields.promocode ||
        data.action.customFields.promocodephone) {
