...
class Mailjet extends CRM {
  ...
  constructor(opt: Params) {
    ...
    try {
      ...
      console.log(
        e.message,
        "can't connect, check MJ_APIKEY_PUBLIC and PRIVATE?",
      );
      process.exit(1);
    }
  ...
  addContactToList = async (email: string): Promise<boolean> => {
    try {
      ...
      console.error(
        `Failed to add contact ${email} to list ${this.list}: ${error.statusCode}`,
      );
      return false;
    }
  };

  updateContactProperties = async (
    message: ActionMessage,
  ): Promise<boolean> => {
    const data = Object.entries(this.contactProperties).map(
      ([sourceField, mailjetField]) => ({
        Name: mailjetField,
    ...
    try {
      ...
      console.error(
        `Updating properties for contact ${message.contact.email}, action ${message.actionId} faile with ${e.statusCode}`,
      );
      return false;
    }
  };
  ...
  handleContact = async (
    ...
    try {
      ...
      const {
        response: { status },
      } = await this.mailjet.post("contact", { version: "v3" }).request({
  ...
  updateContact = async (message: ActionMessage): Promise<boolean> => {
    try {
      const {
        response: { status },
      } = await this.mailjet
      ...
      if (error.statusCode === 304) {
        console.log(
          `${error.statusText} for ${message.contact.email}, ${message.actionId}`,
        );
        return true;
      }
      console.error(
        `Error: ${error.message} for ${message.contact.email}, ${message.actionId}`,
      );
      return false;
    }
  };
